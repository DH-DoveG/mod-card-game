...
func update_card_positions(cards: Array, _duration: float, _offset: Vector3 = Vector3.ZERO):
	...
	for i in range(cards.size()):
		var card = cards[i]
		if card != null:
			#card.animate_to_position(offset + positions[i], duration)
...
func update_card_position(card: Node3D, num_cards: int, index: int, duration: float):
