...
var end = Vector3(4,0,0.2)
var card_width: float = 3.6
var padding: float = 0.15
...
func calculate_card_positions(num_cards: int) -> Array[Vector3]:
	...
	for i in range(num_cards):
		var i_pos: Vector3 = Vector3(start_pos + (i * card_offset), .013 * i, 0)
...
func calculate_card_position_by_index(num_cards: int, index: int):
	...
	return Vector3(start_pos + (index * card_offset), .013 * index, 0)
